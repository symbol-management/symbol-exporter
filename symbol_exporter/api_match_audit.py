...
def main(n_to_pull=100):
    ...
    artifacts = [k for k in requests.get('https://raw.githubusercontent.com/symbol-management'
                                         '/ast-symbol-table/master/.file_listing.json').json() if k.startswith('symbols') and k.partition('/')[2] not in existing_names]
    shuffle(artifacts)
    ...
    for i, artifact in tqdm(enumerate(sorted(artifacts, key=diff_sort)), total=n_to_pull):
        if i >= n_to_pull:
            break
        print(artifact)
        symbols = requests.get(f'https://raw.githubusercontent.com/symbol-management/ast-symbol-table/master/{artifact}').json()
        if not symbols:
            continue
        volume = set()
        ...
        with open(outname, 'w') as f:
            json.dump({'deps': dep_sets, 'bad': list(sorted(bad))}, f, indent=1, sort_keys=True)
